...
function [goodIngredients, badIngredients] = GroupingConcerns (ageGroup, skinType, skinQuirks)
    ...
    % This forms a personalised list ingredients that are beneficial 
    % for the user based on their age group, skin type, and skin concerns,
    % and can be used to guide product recommendations
    goodIngredients = [...
        keywords.good.(ageGroup), ...
        keywords.good.(skinType), ...
        keywords.good.(skinQuirks)];

    %Combine all the bad ingredient cell arrays into one array to form a 
    %personalised list of ingredients to avoid for this user.
    badIngredients = [...
